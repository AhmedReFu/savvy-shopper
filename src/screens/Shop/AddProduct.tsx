import { ADD_PRODUCT, IPA_BASE } from '@env';
...
const SelectBox = ({
    ...
    >
        <Text className="text-[18px] text-[#111827]">{placeholder}</Text>
        {rightIcon}
    </Pressable>
);
...
const AddProduct = () => {
    ...
    const handleSaveChanges = async () => {
        ...
        const returnDaysNum = Number(returnPeriodDays);
        ...
        if (Number.isNaN(returnDaysNum) || returnDaysNum <= 0) {
            toast.show({
        ...
        formData.append('return_period_days', String(returnDaysNum));
        ...
        try {
            ...
            if (res.data?.success === true || res.data?.id) {
                ...
                toast.show({
                    message: res.data?.message || 'Product create failed',
                    type: 'error',
                    style: 'top',
                });
            }
            ...
            toast.show({
                message: error?.response?.data?.message || 'Product create failed',
                type: 'error',
                style: 'top',
            });
        } finally {
                    ...
                    <View className="px-5">
                        <View className="mt-4 bg-white rounded-2xl border border-[#D1D5DB] border-dashed p-6">
                            ...
                            <Text className="text-[14px] text-[#6B7280] text-center mt-2">
                                Clear product image upload korun
                            </Text>
                        ...
                        <View className="mt-6 bg-white border border-[#E5E7EB] rounded-xl px-4 py-4">
                            <View className="flex-row items-center justify-between">
                                ...
                                <Switch value={returnsAccepted} onValueChange={setReturnsAccepted} />
                            </View>
                        </View>
